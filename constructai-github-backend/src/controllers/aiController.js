...
exports.analyzePhoto = async (req, res) => {
  const { projectId, photoUrl, workCategory, siteSection, notes } = req.body;
  ...
  try {
    const projRes = await db.query(`
      SELECT p.*,
        ...
        ) AS current_completion
      FROM projects p WHERE p.id = $1
    `, [projectId]);

    if (projRes.rows.length > 0) {
      project = projRes.rows[0];
      currentCompletion = Number(project.current_completion) || 0;
      blueprint = project.blueprint || 'Standard Warehouse';
    }
  ...
  if (previousPhoto) {
    const isSameUrl = previousPhoto.photo_url === photoUrl;
    const isSameDesc = previousPhoto.description === currentDesc;
    if (isSameUrl || isSameDesc) hasChanges = false;
    ...
    // If > 72 hours (3 days) without change, flag delay
    if (timeSinceLastPhoto > 72 && !hasChanges) hasChanges = false;
  }
  ...
  try {
    ...
    const insertAnalysisQuery = `
      ...
      RETURNING *
    `;
    const analysisRes = await db.query(insertAnalysisQuery, [
      projectId,
      photoUrl,
      stageDetected,
      estimatedCompletion,
...
exports.approveAnalysis = async (req, res) => {
  ...
  try {
    const findRes = await db.query('SELECT * FROM ai_analyses WHERE id = $1', [id]);
    ...
    const projRes = await db.query('SELECT name FROM projects WHERE id = $1', [analysis.project_id]);
    const projectName = projRes.rows.length > 0 ? projRes.rows[0].name : `Project #${analysis.project_id}`;
    ...
    const io = req.app.get('io');
    if (io) {
      const projectName = proj ? proj.name : `Project #${analysis.project_id}`;
      io.to(`project_${analysis.project_id}`).emit('progress_updated', {
        project_id: analysis.project_id,
        completion_percentage: analysis.estimated_completion,
        updated_by_name: req.user ? req.user.fullName : 'System',
    ...
    res.json({
      message: 'AI analysis approved (Offline Simulator)!',
      analysis: { id: Number(id) || Date.now(), ...analysis }
    });
  }
};
